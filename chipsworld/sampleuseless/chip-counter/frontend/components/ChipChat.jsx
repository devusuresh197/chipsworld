  ...
  return (
    <div className="w-full flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-6 duration-500">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
        <div>
          <span className="text-xs uppercase font-extrabold tracking-widest text-emerald-400 flex items-center gap-1.5 mb-1">
          ...
          <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tight flex items-center gap-2">
            💬 <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-300 via-teal-200 to-cyan-300">ChipChat</span> Messenger
          </h3>
        </div>

        <span className="px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-bold shadow-sm flex items-center gap-1.5">
          <Circle className="w-2 h-2 fill-emerald-400 text-emerald-400 animate-ping" />
          Live Snack Bowl Network
        </span>
      </div>

      {/* WhatsApp / iMessage Container */}
      <div className="w-full h-[540px] glass-card rounded-3xl border border-white/15 overflow-hidden grid grid-cols-1 md:grid-cols-3 shadow-2xl bg-slate-950/80 backdrop-blur-xl">
        
        {/* Left Sidebar - Contacts List */}
        <div className="md:col-span-1 border-r border-white/10 flex flex-col bg-slate-900/60">
          {/* Sidebar Search Bar */}
          <div className="p-3 border-b border-white/10 bg-slate-950/40">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900/80 border border-white/10 rounded-xl pl-9 pr-3 py-2 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500/50"
              />
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full p-3 rounded-2xl flex items-center gap-3 text-left transition-all duration-200 cursor-pointer ${
                    isSelected
                      ? "bg-emerald-600/20 border border-emerald-500/40 shadow-md"
                      : "hover:bg-white/5 border border-transparent"
                  }`}
                >
                  <div className="relative flex-shrink-0">
                    <div className="w-10 h-10 rounded-full bg-slate-950 border border-white/20 flex items-center justify-center text-xl shadow-inner">
                      {avatarEmoji}
                    </div>
        ...
        <div className="md:col-span-2 flex flex-col h-full bg-slate-950/90 relative">
          ...
          {/* Active Chat Header */}
          <div className="p-3.5 px-5 border-b border-white/10 bg-slate-900/80 backdrop-blur-md flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-slate-950 border border-emerald-500/40 flex items-center justify-center text-xl shadow-sm">
                🥔
              </div>
              <div>
                <h4 className="text-sm font-bold text-white flex items-center gap-2">
                  {activeChip.name}
                  <span className="text-[10px] text-indigo-300 font-mono font-normal">({activeChip.username})</span>
                </h4>
            ...
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-[10px] font-bold font-mono">
                {activeChip.id}
              </span>
            </div>
          </div>
          ...
          <form onSubmit={handleSendMessage} className="p-3 border-t border-white/10 bg-slate-900/90 flex items-center gap-2">
            <input
              type="text"
              placeholder={`Message ${activeChip.name}... (e.g. "Stay crispy!")`}
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              className="flex-1 bg-slate-950 border border-white/15 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500/60 transition-colors"
            />
            ...
            <button
              ...
              className={`p-2.5 rounded-xl font-bold transition-all duration-200 flex items-center justify-center ${
                inputText.trim()
                  ? "bg-emerald-600 hover:bg-emerald-500 text-white cursor-pointer shadow-lg hover:scale-105"
